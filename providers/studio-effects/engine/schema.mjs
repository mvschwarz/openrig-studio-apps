...
export const FAMILIES = {
  mosh: {
    ...
    params: {
      ...
      sliceLoss:    { type: "float", min: 0, max: 1, default: 0, pairsWith: ["sliceHeight", "sliceHold"],
                      says: "share of horizontal slices that never arrive; a lost slice gets NO correction, so it freezes and slides while the rest of the picture recovers" },
      sliceHeight:  { type: "int", min: 1, max: 16, default: 3, pairsWith: ["sliceLoss"],
                      says: "how many macroblocks tall a slice is — small values give thin scattered bands, large ones drop whole regions" },
      sliceShift:   { type: "float", min: 0, max: 64, default: 0, pairsWith: ["sliceLoss"],
                      says: "sideways desync of a broken slice, in pixels — the torn, horizontally-displaced band of a stream that lost sync mid-row" },
      sliceHold:    { type: "int", min: 1, max: 30, default: 4, pairsWith: ["sliceLoss"],
                      says: "frames a loss persists before new slices are dropped; 1 re-rolls every frame and reads as noise rather than as a stream in trouble" },
    ...
    highlights: {
      ...
      "broken stream":     ["sliceLoss", "sliceHeight", "sliceShift"],
    },
    ...
    presets: {
      ...
      "broken stream":    { show: "mosh", refresh: 0.06, residual: 0.5, sliceLoss: 0.35, sliceHeight: 3, sliceHold: 5, searchRadius: 20 },
      "lost signal":      { show: "mosh", refresh: 0.02, residual: 0.25, sliceLoss: 0.6, sliceHeight: 2, sliceHold: 8, sliceShift: 24, searchRadius: 24 },
      "torn slices":      { show: "mosh", refresh: 0.1, residual: 0.5, sliceLoss: 0.45, sliceHeight: 1, sliceHold: 3, sliceShift: 48 },
  ...
  codec: {
    id: "codec",
    name: "Codec",
    summary: "The DCT round trip a JPEG performs. The picture is cut into blocks, turned into frequency coefficients, those coefficients are rounded away against a real quantisation table, and it is all inverted. The blocking, the ringing at edges and the colour smear are what the rounding threw away — none of it is drawn on.",
    engine: "webgl2",
    params: {
      ...
      seed:             { type: "int", min: 0, max: 9999, default: 7,
                          says: "which blocks get the jitter and bias" },
    },
    guidance: [
      "'compressed' or 'low quality' — drop quality to 10-20 and leave everything else alone; that is what a real bad encode looks like.",
      "'the colour is wrong' — drop chromaQuality alone and keep quality high. Colour always goes first in real compression and this shows why.",
    ...
    presets: {
      ...
      "colour goes first":   { quality: 88, chromaQuality: 3, subsample: "4:2:0" },
